...
namespace glTFRuntimeAlembic
{
	...
	struct GLTFRUNTIMEALEMBIC_API IProperty : public TSharedFromThis<IProperty>
	{
		...
		static TSharedPtr<IProperty> BuildProperty(const TSharedRef<FOgawaData>& Headers, uint64& Offset, const TSharedPtr<IOgawaNode>& PropertyNode);
	};
	...
	struct GLTFRUNTIMEALEMBIC_API FObject : public TSharedFromThis<FObject>
	{
		...
		static TSharedPtr<FObject> BuildObject(const TSharedPtr<FObject>& Parent, const FString& Name, const TMap<FString, FString>& Metadata, const TSharedPtr<FOgawaGroup>& Group);
